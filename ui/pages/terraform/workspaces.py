...
class CreateWorkspaceModal(BasePage):
    ...
    def __init__(self):
        self.element = s(by.xpath('//div[starts-with(@id, "workspaceform") and contains(@id, "body")]/ancestor::div'))
        self.form_main = ss('div[id^="workspaceform"] > fieldset')[0]
        self.form_additional = ss('div[id^="workspaceform"] > fieldset')[1]

    @property
    def name(self) -> input:
        return input(label='Name', parent=self.element)

    @property
    def vcs_provider(self) -> combobox:
        return combobox('VCS Provider', self.element)

    @property
    def repository(self) -> combobox:
        return combobox('Repository', self.element)

    @property
    def terraform_version(self) -> combobox:
        return combobox('Terraform Version', self.element)

    @property
    def auto_apply(self) -> toggle:
        return toggle('Auto Apply', self.element)

    def toggle_additional(self) -> SeleneElement:
        return self.element.s('div[componentid="additional-legendToggle"]').click()
    ...
    def subdirectory(self) -> input:
        return input('Subdirectory', self.element)

    @property
    def work_directory(self) -> input:
        return input('Terraform Work Directory', self.element)

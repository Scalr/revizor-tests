...
class TestWorkspaceRun:
    ...
    def wait_run_queued(self):
        loading_modal(consts.LoadingModalMessages.QUEUEING_RUN).should(be.visible, timeout=5)
    ...
    def workspace(self,subdirectory=None):
        self.workspace_name = generate_name('name')
        self.tf_version = '0.12.19'
        provider_id = self.vcs_provider['id']
        IMPL.workspace.create(self.workspace_name, self.tf_version, provider=provider_id, repository='Scalr/tf-revizor-fixtures', branch='master', patch=subdirectory)
        time.sleep(1)
    ...
    def test_approve_run(self):
        run_page = self.workspace('local_wait')
        time.sleep(1)
        self.run_page.queue_run.click()
        confirm = QueueNewRunModal()
        confirm.queue_button.click()
        self.wait_run_queued()
        assert len(self.run_page.workspace_run) > 0
        self.run_page.refresh.click()
        time.sleep(1)
        run_line = self.run_page.workspace_run[0]
        assert run_line.carrent.get(query.text) == 'CURRENT'

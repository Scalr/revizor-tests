...
def pytest_configure(config):
    revizor_path = pathlib.Path(CONF.logging_path) / 'scalarizr' / f"test_{datetime.now().strftime('%m%d-%H.%M.%S')}.log"
    config.pluginmanager.get_plugin("logging-plugin").set_log_path(revizor_path.expanduser().absolute())
...
def testenv(request) -> TestEnv:
    ...
    try:
        ...
        if (
            ...
            and (
                new_te
                and request.session.testsfailed == 0
                or request.config.getoption("--te-remove")
            )
        ):
...
def farm(request: FixtureRequest) -> Farm:
    if CONF.main.farm_id is None:
        ...
        test_farm = Farm.create(
            f'tmprev-{datetime.now().strftime("%d%m%H%M%f")}',

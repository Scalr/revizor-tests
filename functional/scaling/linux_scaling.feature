...
Feature: Linux server read/execute scaling test

    @ec2 @gce @cloudstack @rackspaceng @openstack @eucalyptus
    Scenario: Bootstraping execute scaling
        Given I have a clean and stopped farm
        And I add role to this farm with prepare_scaling_linux,scaling_execute_linux
        When I start farm
    ...
    Scenario: Test execute scaling up
        When I set file '/tmp/revizor_scaling.txt' content to 100 on M1
        Then I expect server bootstrapping as M2
        And I set file '/tmp/revizor_scaling.txt' content to 60 on M1
        And not ERROR in M1 scalarizr log
        And not ERROR in M2 scalarizr log

    @ec2 @gce @cloudstack @rackspaceng @openstack @eucalyptus
    Scenario: Test execute scaling down
        When I set file '/tmp/revizor_scaling.txt' content to 0 on M1
    ...
    Scenario: Bootstraping read scaling
        Given I have a clean and stopped farm
        And I add role to this farm with scaling_read_linux
        When I start farm
        Then I expect server bootstrapping as M1
        And scalarizr version from system repo is last in M1

...
@step(r"I add (http|https|http/https) proxy (\w+) to (\w+) role with ([\w\d]+) host to (\w+) role")
def add_proxy_with_role(step, proto, proxy_name, proxy_role, vhost_name, backend_role):
    """
    ...
    proxy_role.add_proxy(vhost, port, backends=backends, **opts)
...
def modify_proxy(step, proxy, role, ip_hash, options):
    ...
    role.edit_proxy(proxy['hostname'], proxy['port'], backends, ip_hash=ip_hash)


@step(r"I delete proxy ([\w\d]+) in ([\w\d]+) role")
def delete_proxy(step, proxy, role):
    proxy = getattr(world, '%s_proxy' % proxy)
    role = getattr(world, '%s_role' % role)
    role.delete_proxy(proxy['hostname'])

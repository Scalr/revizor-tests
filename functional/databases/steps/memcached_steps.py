...
def add_new_item_to_memcached(step, commands, command, serv_as):
    ...
    if not result:
        raise AssertionError("Suggested command is not added an element to memcache: world.memcache_client.%s('1000','test_value')"
                             % str(world.memcache_maps[commands][command]))
...
def get_item_from_memcached(step, commands, command, serv_as):
    ...
    if result != "test_value":
        raise AssertionError("Suggested command is not get element from memcache: world.memcache_client.%s('1000')"
                             % str(world.memcache_maps[commands][command]))

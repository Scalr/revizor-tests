...
def check_instance_configs(step, instance_port, search_condition, serv_as):
    ...
    if search_condition == 'configuration':
        path = redis_path.get(node.os.family)['conf']
        file = 'redis.{0}.conf'.format(instance_port)
    else:
        path = redis_path.get(node.os.family)['data']
    ...
    LOG.debug('The result: ({0}) of the find command for redis instance {1}'.format(res.std_out, instance_port))
    assertion_message = 'Redis instance {0} config file ({1}) was not deleted.'.format(
        instance_port,
        res.std_out)
    assert not (res.std_out), assertion_message

...
Feature: Redis database server role, api tests
    ...
    Scenario: Reset instances passwords
        When I run "RedisApi" command "reset_password" on M1 with arguments:
            | port | new_password      |
            | 6379 | INSTANCE-PASSWORD |
        And I run "RedisApi" command "list_processes" on M1
        Then password for instance 6379 was changed on M1
        When I run "RedisApi" command "reset_password" on M1 with arguments:
            | port |
            | 6380 |
        And I run "RedisApi" command "list_processes" on M1
        Then password for instance 6380 was changed on M1
        And not ERROR in M1 scalarizr log
    ...
    Scenario: Launch additional redis instances
        ...
        Then I run "RedisApi" command "get_service_status" on M1
        And 6379,6380,6381,6382,6383 redis instance is running
        And not ERROR in M1 scalarizr log
    ...
    Scenario: Change redis-server instances status (stop, shutdown)
        ...
        When I write test data to 6379 redis instance on M1
        And I write test data to 6380 redis instance on M1
        Then I run "RedisApi" command "stop_service" and pid has been changed on M1 with arguments:
            | ports |
            | 6379  |
        ...
        And redis instance with port 6380 has not database on M1
        Then I run "RedisApi" command "list_processes" on M1
    ...
    Scenario: Change redis-server instances status (start, restart)
        Given I run "RedisApi" command "start_service" and pid has been changed on M1 with arguments:
            | ports  |
            | [6379] |
        ...
        Then I run "RedisApi" command "get_service_status" on M1
        And 6379,6381,6382,6383 redis instance is running
        Then I read from 6379 redis instance, and test data exists on M1
        And not ERROR in M1 scalarizr log
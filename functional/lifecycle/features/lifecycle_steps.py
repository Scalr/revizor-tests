...
@step('I see (.+) server')
def waiting_for_assertion(step, spec, timeout=1400):
	if CONF.main.platform == 'ucloud':
		timeout = 2000
	world.server = wait_until(world.check_server_status, args=(spec, world.role.role_id), timeout=timeout, error_text="I'm not see this %s state in server" % spec)
	LOG.info('Server succesfull %s' % spec)
	
@step('I wait and see (.+) server')
def waiting_server(step, spec, timeout=1400):
	if CONF.main.platform == 'ucloud':
		timeout = 2000
	world.server = wait_until(world.check_server_status, args=(spec, world.role.role_id), timeout=timeout, error_text="I'm not see this %s state in server" % spec)
	LOG.info('Server succesfull %s' % spec)
...
@step('I have (.+) (.+) server')
def having_server(step, state, os):
	world.assert_not_equal(world.server.status, ServerStatus.from_code(state), "Server %s is not in state %s" % (world.server.id, state))


@step("directory '(.+)' exist")
def check_path(step, path):
	c = Cloud()
	node = c.get_node(world.server)
...
@step('script output contains (.+)')
def assert_check_message_in_log(step, message):
	time.sleep(60)
	world.server.scriptlogs.reload()
	for log in world.server.scriptlogs:
		LOG.debug('Server %s log content: %s' % (world.server.id, log.message))
...
@step('I execute \'(.+)\'')
def execute_command(step, command):
	c = Cloud()
	world.node = c.get_node(world.server)
	LOG.info('Execute command on server: %s' % command)
	world.node.run(command)


@step('server contain \'(.+)\'')
def check_file(step, path):
	out = world.node.run('ls %s' % path)

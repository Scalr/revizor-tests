...
Feature: Check chef attributes set
    ...
    Scenario: Chef boot support systemd
        Given I have a clean and stopped farm
        When I add role to this farm with chef
        When I start farm
        Then I expect server bootstrapping as M1
        ...
        Then I verify that this value 15 appears in the startup line on M1

    @ec2 @gce @cloudstack @openstack @rackspaceng @systemd
    Scenario: Verify that chef-client is running and it runs smooth
        Given I wait and see that chef-client runs more than INTERVAL

...
def assert_server_message_count(step, msgtype, msg, serv_as, timeout=1500):
    ...
    income_messages = [m.name for m in server.messages if m.type == 'in' and  m.name == msg]
    messages_count = len(income_messages)
    ...
    assert messages_count == mount_device_count, (
        'Scalr internal messages count %s != %s Mounted storages count. List of all Incoming msg names: %s ' % (
            message_count, mount_device_count, income_messages))

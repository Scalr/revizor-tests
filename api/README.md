...
--te-creation-timeout или --te-ct: Устанавливает время доступа к запущенному ранее или создаваемому контейнеру,
по истечении указанного времени тест считается не пройденным. Значение по умолчанию: 300
...
def api(request, fileutil):
    ...
    return session
```
Данная фикстура является инстансом `AppSession` класса имплементирующего функционал api вызов и верификации request/response на
...
.
```
Переменная может иметь одно из следующих значений `(user|account|global|system)`. `user` является дефолтным значением и
